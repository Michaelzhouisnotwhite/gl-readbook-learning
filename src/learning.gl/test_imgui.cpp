...
int main(int, char**) {
    ...
    // - Our Emscripten build process allows embedding fonts to be accessible at runtime from
    // the "fonts/" folder. See Makefile.emscripten for details.
    ...
    // Main loop
#ifdef __EMSCRIPTEN__
    // For an Emscripten build we are disabling file-system access, so let's not attempt to do
    // a fopen() of the imgui.ini file. You may manually call LoadIniSettingsFromMemory() to
    // load settings from your own storage.
...
#endif
    {
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create
        // a named window.
        {
            static float f = 0.0f;
